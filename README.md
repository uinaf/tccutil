...
- Default `reset SERVICE` (no client) clears matching rows in both databases in one SQLite transaction; a statement failure rolls back both, and crash-atomicity under write-ahead logging (WAL) is not guaranteed.
- Default `reset SERVICE` may require `sudo` when the live system database has matches.
- If the system database exists but cannot be inspected (common without Full Disk Access), default reset fails closed; use `--user` to reset only the per-user database.
